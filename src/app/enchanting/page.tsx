...
export default function EnchantingPlanner() {
  ...
  const [exportTab, setExportTab] = useState<'auctionator'|'tsm'>('auctionator');
            ...
            <>
              ...
              <div className="flex flex-col items-center mt-2">
                <div className="flex gap-2 mb-2">
                  <button
                    className="bg-green-700 hover:bg-green-800 text-white px-3 py-1 rounded text-xs font-semibold"
                    onClick={() => setShowMaterialSummary(v => !v)}
                  >
                    {showMaterialSummary ? 'Hide' : 'Show'} Total Materials Needed
                  </button>
                  {showMaterialSummary && (
                    <button
                    ...
                    >
                      {showExport ? 'Hide Export' : 'Export Materials'}
                    </button>
                  )}
                </div>
                {showMaterialSummary && (
                  <section className="bg-neutral-900 mt-2 p-2 rounded text-xs w-full">
                    <h3 className="font-bold mb-2 text-green-300">Total Materials Needed (Optimal Route)</h3>
                    <MaterialSummary steps={steps} materialMap={materialMap} prices={prices} />
                  </section>
                )}
                {showMaterialSummary && showExport && (
                ...
                {Object.entries(materialTotals).map(([id, {qty, cost}]) => {
                    ...
                    return (
                      <tr key={id}>
                        ...
                        <td className="py-1 text-right">{Number(qty)}</td>
...
function MaterialSummary({ steps, materialMap, prices }: { steps: any[], materialMap: Record<string, string>, prices: any }) {
  // Aggregate total materials
  const totals: Record<string, number> = {};
  steps.forEach(step => {
    for (const [id, qty] of Object.entries(step.recipe.materials)) {
      totals[id] = (totals[id] || 0) + qty * step.crafts;
    }
  });
  if (Object.keys(totals).length === 0) return <p className="text-neutral-400">No materials required.</p>;
  return (
...
function ExportMaterialsModal({ steps, materialMap, exportTab, setExportTab }: {
  steps: any[],
  materialMap: Record<string, string>,
  exportTab: 'auctionator'|'tsm',
  setExportTab: (tab: 'auctionator'|'tsm') => void
}) {
  // Aggregate total materials
  const totals: Record<string, number> = {};
  steps.forEach(step => {
    for (const [id, qty] of Object.entries(step.recipe.materials)) {
      totals[id] = (totals[id] || 0) + Number(qty) * step.crafts;
    }
  });
  // Prepare export strings
  const auctionator = Object.entries(totals)
    .map(([id, qty]) => `${materialMap[id] ?? `Item ${id}`} x${qty}`)
    .join('\n');
  const tsm = Object.entries(totals)
    .map(([id, qty]) => `${materialMap[id] ?? `Item ${id}`};${qty}`)
    .join('\n');
  // Copy to clipboard
  ...
  return (
    <div className="bg-neutral-800 border border-neutral-700 rounded p-4 mt-2 w-full max-w-lg">
      <div className="flex gap-2 mb-2">
        <button
          className={`px-2 py-1 rounded text-xs font-semibold ${exportTab==='auctionator' ? 'bg-green-700 text-white' : 'bg-neutral-700 text-neutral-300'}`}
          onClick={() => setExportTab('auctionator')}
        >Auctionator</button>
        <button
          className={`px-2 py-1 rounded text-xs font-semibold ${exportTab==='tsm' ? 'bg-blue-700 text-white' : 'bg-neutral-700 text-neutral-300'}`}
          onClick={() => setExportTab('tsm')}
        >TSM</button>
      </div>
      <textarea
        className="w-full h-32 p-2 rounded bg-neutral-900 text-neutral-100 mb-2"
        readOnly
        value={exportTab==='auctionator' ? auctionator : tsm}
      />
      <button
        className="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-xs font-semibold"
        onClick={() => copy(exportTab==='auctionator' ? auctionator : tsm)}
